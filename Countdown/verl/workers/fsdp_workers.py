...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        if self._is_actor and optim_config is not None:
            # print(dict(actor_module_fsdp.named_parameters()).keys())
            from verl.utils.torch_functional import get_constant_schedule_with_warmup
            actor_optimizer = optim.AdamW(
                # actor_module_fsdp.parameters(),
                [{'params':actor_module_fsdp.parameters(),'lr':optim_config.lr}],#,#(actor_module_fsdp.parameters(),#
            # lr=optim_config.lr,
        ...
        return actor_module_fsdp, actor_optimizer, actor_lr_scheduler, actor_model_config, None#scale_fsdp
    ...
    def init_model(self,path=None):
        ...
        from verl.workers.actor import DataParallelROVERActor
        ...
        if self._is_actor or self._is_rollout:
            ...
            self.actor_module_fsdp, self.actor_optimizer, self.actor_lr_scheduler, self.actor_model_config, self.scale_fsdp = self._build_model_optimizer(
        ...
        if self._is_actor:
            ...
            self.actor = DataParallelROVERActor(config=self.config.actor,
        ...
        if self._is_ref:
        # if True:
            self.ref_module_fsdp = self._build_model_optimizer(model_path=self.config.model.path if path==None else path,
    ...
    def update_actor(self, data: DataProto, scale=None):
        data = data.to('cuda')
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        if self._is_actor and recompute_log_prob:
            ...
            with self.ulysses_sharding_manager:
                ...
                old_log_probs, old_logits = self.actor.compute_log_prob(data=output)
                output.batch['old_log_probs'] = old_log_probs
                output.batch['old_logits'] = old_logits
                # output.batch['old_logits_m'] = old_logits_m
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            output, logits = self.ref_policy.compute_log_prob(data=data, is_ref=True)
            output = DataProto.from_dict(tensors={'ref_log_prob': output,'sum_ref_logits':logits})
        ...
        return output
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_scale(self, local_path, hdfs_path=None):
        assert self._is_actor
        import torch
        if self._is_offload_param:
            load_fsdp_param_and_grad(module=self.scale_fsdp,
                                     device_id=torch.cuda.current_device(),
                                     load_grad=self._is_offload_grad)
        ...
        with FSDP.state_dict_type(self.actor.scale, StateDictType.FULL_STATE_DICT, cfg):
            scale_state_dict = self.actor.scale.state_dict()
        if self.rank == 0:
            os.makedirs(local_path, exist_ok=True)
            torch.save(scale_state_dict, f'{local_path}_scale.pt')
        torch.distributed.barrier()
        if self._is_offload_param:
            offload_fsdp_param_and_grad(module=self.scale_fsdp, offload_grad=self._is_offload_grad)
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_checkpoint(self, local_path, hdfs_path=None):
        ...
        if self.rank == 0:
            ...
            os.makedirs(local_path, exist_ok=True)
            # torch.save(scale_state_dict, f'{local_path}/scale.pt')
