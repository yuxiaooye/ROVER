...
def qwen2_dtensor_weight_loader(actor_weights: Dict, vllm_model: nn.Module) -> nn.Module:
    ...
    # params_dict['model.q_head_proj'] = actor_weights['model.q_head_proj'].to(dtype=list(params_dict.values())[0].dtype,device=list(params_dict.values())[0].device)#torch.rand((1024,),device=list(params_dict.values())[0].device,dtype=list(params_dict.values())[0].dtype)
    ...
    for name, loaded_weight in actor_weights.items():
        ...
        for param_name, weight_name, shard_id in stacked_params_mapping:
            ...
            # if name == 'model.q_head_proj':
            #     assert param.size() == loaded_weight.size()
            #     # print('!!!!!',param.size(),param[0],local_loaded_weight[0])
            #     # param = local_loaded_weight.to(dtype=param.dtype)
            #     param.data.copy_(local_loaded_weight.to(dtype=param.dtype))
            # else:
                # weight_loader = getattr(param, "weight_loader", default_weight_loader)
                # weight_loader(param, local_loaded_weight.to(dtype=param.dtype))
