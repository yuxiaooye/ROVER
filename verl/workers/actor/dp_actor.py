...
from verl.utils.torch_functional import logprobs_from_logits, sum_log_softmax, masked_mean
...
class DataParallelRPEActor(BasePPOActor):
    ...
    def __init__(self, config: ActorConfig, actor_module: nn.Module, actor_optimizer: torch.optim.Optimizer = None):
        ...
        self.compute_sumlog = torch.compile(verl_F.sum_log_softmax, dynamic=True)
        self.device_name = get_device_name()
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                if self.use_fused_kernels:
                    ...
                    q_value_rmpad = self.compute_sumlog(logits_rmpad)
                ...
                if self.use_ulysses_sp:
                    ...
                    q_value_rmpad = gather_outpus_and_unpad(q_value_rmpad,
                                        gather_dim=0,
                                        unpad_dim=0,
                                        padding_size=pad_size)
                # pad back to (bsz, seqlen)
                ...
                full_q_values = pad_input(hidden_states=q_value_rmpad.unsqueeze(-1),
                                           indices=indices,
                                           batch=batch_size,
                                           seqlen=seqlen)
            ...
            return entropy, log_probs, q_values
        ...
        for in_epoch in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    entropy, log_prob, current_q = self._forward_micro_batch(
                        model_inputs, temperature=temperature, calculate_entropy=calculate_entropy
                    )
                    ...
                    if hasattr(self.config, 'use_rpe_clip') and self.config.use_rpe_clip:
                        ...
                        # print('>>> clip_low', clip_low)
                        
                        condition_high = (target > logIS) & (logIS > clip_high)
                        condition_low = (target < logIS) & (logIS < clip_low)
                        
                        total_valid = response_mask.sum().float() + 1e-8
                        clip_high_count = torch.sum(condition_high & response_mask).float()
                        clip_low_count = torch.sum(condition_low & response_mask).float()
                        ...
                        q_values = clipped_ratio * response_mask
                        
                        metrics_update = {
                            'actor/clip_high_ratio': (clip_high_count / total_valid).detach().item(),
                            'actor/clip_low_ratio': (clip_low_count / total_valid).detach().item(),
                        }
                        append_to_dict(metrics, metrics_update)
                    else:
                        q_values = logIS * response_mask #(log_prob - 0.9*old_log_prob-0.1*ref_log_prob) #+ log_prob - ref_log_prob_

                    metrics_update = {
                        'actor/IS_max': torch.exp(logIS).max().detach().item(),
                        'actor/IS_mean': torch.exp(logIS).mean().detach().item(),
                        'actor/IS_masked_mean': masked_mean(torch.exp(logIS), response_mask).detach().item(),
                        'actor/IS_min': torch.exp(logIS).min().detach().item(),
                        'actor/logIS_max': logIS.max().detach().item(),
                        'actor/logIS_mean': logIS.mean().detach().item(),
                        'actor/logIS_masked_mean': masked_mean(logIS, response_mask).detach().item(),
                        'actor/logIS_min': logIS.min().detach().item(),
                        'actor/q_values_max': q_values.max().detach().item(),
                        'actor/q_values_mean': q_values.mean().detach().item(),
                        'actor/q_values_masked_mean': masked_mean(q_values, response_mask).detach().item(),
                        'actor/q_values_min': q_values.min().detach().item(),
                    }
                    append_to_dict(metrics, metrics_update)
