...
def compute_rover_outcome_advantage(token_level_rewards: torch.Tensor,
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            if scores[i]>0:
                scores[i] = 0+5*((1-sum(scores_copy>0)/bsz))
            else:
                scores[i] = 0-2.*((1-sum(scores_copy<=0)/bsz))  
...
def compute_reinforce_plus_plus_baseline_outcome_advantage(
    ...
    **kwargs,
    ...
    # print('>>>REINFORCE_PLUS_PLUS_BASELINE compute_advantage')
    """
...
def compute_reinforce_plus_plus_outcome_advantage(
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape: (bs, response_length)
    """
    # print('>>>REINFORCE_PLUS_PLUS compute_advantage')
    assert config is not None
    gamma = config.gamma
    with torch.no_grad():
...
def compute_policy_loss_vanilla(
    ...
    # print('>>>DAPO clip higher, clip_ratio_high=', clip_ratio_high, 'clip_ratio_low=', clip_ratio_low)
