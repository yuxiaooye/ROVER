...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            if hasattr(self.config, 'is_ppo') and self.config.is_ppo:
                # print('>>>self.actor, use DataParallelPPOActor')
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        if hasattr(self.config, 'is_ppo') and self.config.is_ppo:
            ...
            with self.ulysses_sharding_manager:
                with adapter_ctx:
                    output, entropys, old_logits = self.actor.compute_log_prob(data=data, calculate_entropy=True)
                output = DataProto.from_dict(
                    tensors={"old_log_probs": output, "entropys": entropys, "old_logits": old_logits},
                    meta_info={"temperature": self.config.rollout.temperature},
                )
