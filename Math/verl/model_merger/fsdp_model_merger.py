...
if __name__ == "__main__":
    ...
    config = ModelMergerConfig(
        operation="merge",
        backend="fsdp",
        local_dir=os.path.join(args.global_step_dir, "actor"),
        target_dir=os.path.join(args.global_step_dir, "actor_merged"),
        hf_model_config_path=os.path.join(args.global_step_dir, "actor/huggingface")
    )
    merger = FSDPModelMerger(config)
    merger.merge_and_save()